...
bool loadSummary(csim_stats_t *stats) {
    ...
    if (fscanf(fp, "%ld %ld %ld %ld %ld", &stats->hits, &stats->misses,
 ...
 */
void copyMatrix(size_t M, size_t N, double Adst[N][M],
                const double Asrc[N][M]) {
    size_t i, j;
    for (i = 0; i < N; i++)
        for (j = 0; j < M; j++)
            Adst[i][j] = Asrc[i][j];
 ...
 */
void registerTransFunction(void (*trans)(size_t M, size_t N, const double[N][M],
                                         double[M][N], double *T),
                           const char *desc) {
